...
"$HOST_CXX" "${COMMON_FLAGS[@]}" "$ROOT/tests/enemy_economy_armor_tests.cpp" "$ROOT/source/Economy.cpp" "$ROOT/source/Enemy.cpp" "$ROOT/source/Level.cpp" "$ROOT/source/Projectile.cpp" "$ROOT/source/Wave.cpp" -o "$BUILD_DIR/enemy-economy-armor-tests"
...
"$BUILD_DIR/enemy-economy-armor-tests"
...
grep -q "appendEnemy(vertices, enemyTypes\[type\], flash != 0U)" "$ROOT/source/Renderer.cpp"
grep -q "enemyVertexOffsets_\[type\]\[flash\]" "$ROOT/source/Renderer.cpp"
grep -q "EnemyVisual::shouldShowHealthBar" "$ROOT/source/Renderer.cpp"
grep -q "healthBarFillOffsets_\[bucket\]" "$ROOT/source/Renderer.cpp"
