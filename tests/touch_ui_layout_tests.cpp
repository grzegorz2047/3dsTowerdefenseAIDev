...
void testAllActionsHaveUsableHitboxes() {
    ...
    for (const TouchUiAction action : actions) {
        expectCenter(action);
    }
}
...
void testEdgesAreHalfOpen() {
    ...
    expect(
        TouchUiLayout::actionAt(static_cast<std::int16_t>(rect.x + rect.width), rect.y) != TouchUiAction::SelectBallista,
        "right edge should belong outside the button");
    expect(
        TouchUiLayout::actionAt(rect.x, static_cast<std::int16_t>(rect.y + rect.height)) != TouchUiAction::SelectBallista,
